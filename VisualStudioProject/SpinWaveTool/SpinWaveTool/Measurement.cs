    ...
    public class Measurement : IDisposable
    {
        ...
        public class Data
        {
            ...
            public int bandwidth
            {
                set
                {
                    _bandwidth = Math.Clamp(value, 1, 999);
                }
                get
                {
                    return _bandwidth;
            ...
            public int reloads
            {
                set
                {
                    _reloads = Math.Clamp(value, 1, 16);
                }
                get
                {
                    return _reloads;
            ...
            public int reload_delay
            {
                set
                {
                    _reload_delay = Math.Clamp(value, 50, 9999);
                }
                get
                {
            ...
            public void Load(string file)
            {
                ...
                for (var i = 0; i < 4; i++)
                {
                    var parameter = Enum.Parse<VNA.CalcParameter>(data["vna"]["parameter_" + i]);
                    var format = Enum.Parse<VNA.DataFormat>(data["vna"]["format_" + i]);
